...
4. Append a plugin-spec override that disables Mason (`mason.nvim`, `mason-lspconfig.nvim`) and zeros out nvim-treesitter's `ensure_installed` (Mason can't run on NixOS — downloads pre-built FHS-expecting binaries; treesitter parsers come from Nix).
5. Patch lazy.nvim's docs task to skip helptags generation for dev plugins (lazy regenerates helptags for local plugins on every install/update, but the Nix-supplied copy is read-only and ships prebuilt `doc/tags`).
